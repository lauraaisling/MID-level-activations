...
def plot_dsprites_images(images, ncols=5, title=None, annotations=None, 
                         centers=None, keep_colors=False):
    """
    ...
    for i, image in enumerate(images):
        if image is not None and len(image.shape) == 3:
            if not keep_colors: 
                ...
                images[i] = np.moveaxis(image, 0, -1)

    num_images = len(images)
    ncols = np.min([num_images, ncols])
    nrows = int(np.ceil(num_images / ncols))
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, 
        figsize=(ncols * 2.2, nrows * 2.2), squeeze=True, constrained_layout=True # 2.7 squeeze=True False
        )

    if title is not None:
        fig.suptitle(title, y=1.04) # , y=1.04, pad=
