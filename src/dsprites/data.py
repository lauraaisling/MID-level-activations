...
DEFAULT_DATASET_NPZ_PATH = os.path.join(os.environ.get("LIB_DATA", "."), "dsprites", "dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz")
...
def train_test_split_idx(dataset, fraction_train=0.8, 
                         randst=None, train_pos_bias=None, control=False, 
                         fraction_pos_bias = None, num_samples=None): 
    """
    ...
    - randst (torch Generator or int): random state to use when splitting 
        dataset. (default: None)
    - train_pos_bias (str): type of bias to introduce into the training dataset, 
    ...
    - control (bool): if True, the same number of items are removed from the 
        training dataset as the train_pos_bias would determine, but they are 
        randomly selected. (default: False)
    - fraction_pos_bias (float): Proportion of samples you want sample and control.
        TODO test 
    - num_samples (int): number of samples to train and test combined on
        (proportion of full dataset) 
        TODO test complicated functionality (e.g., biased)
    ...
    if num_samples is None: 
        ...
        train_size = int(fraction_train * num_samples)
    ...
    if train_pos_bias is not None:
        ...
        if fraction_pos_bias is not None: 
            if 1 <= fraction_pos_bias <= 0:
            ...
            train_indices_bias = get_biased_indices(
                dataset, train_indices, bias=train_pos_bias, control=False
                )
            train_indices_control = get_biased_indices(
                dataset, train_indices, bias=train_pos_bias, control=True
                )
            fraction_control = 1.0 - fraction_pos_bias
            # keep all of larger proportion and discard appropriate amount of the other
            if fraction_pos_bias > 0.5: 
                size = int(len(train_indices_control) * (fraction_control/fraction_pos_bias))
                train_indices_control = train_indices_control[:size]
            elif fraction_pos_bias < 0.5: 
                size = int(len(train_indices_bias) * (fraction_pos_bias/fraction_control))
                train_indices_bias = train_indices_bias[:size]

            # print(train_indices_bias)
            train_indices = torch.cat((train_indices_bias, train_indices_control))
            # print(train_indices)
        else: 
            train_indices = get_biased_indices(
                dataset, train_indices, bias=train_pos_bias, control=control
                )
    test_indices = all_indices[train_size :]

    # doesn't seem to be shuffeling below without this
    train_indices = train_indices[torch.randperm(len(train_indices), generator=randst)]
    test_indices = test_indices[torch.randperm(len(test_indices), generator=randst)]
    ###

    train_sampler = torch.utils.data.SubsetRandomSampler(train_indices)
    test_sampler = torch.utils.data.SubsetRandomSampler(test_indices)

    return train_sampler, test_sampler, train_indices, test_indices
...
class dSpritesDataset():
    ...
    def show_images(self, indices=None, num_images=10, randst=None, 
        ...
        shape_names = self.get_shapes_from_values(latent_values[:, 1])
        ...
        for ax_i, ax in enumerate(axes.flatten()):
            if ax_i < num_images:
                ...
                img_latent_values[1] = \
                    f"{latent_values[ax_i, 1]} ({shape_names[ax_i]})"
                if not (ax_i % ncols):
                    title = "\n".join(
...
def plot_dsprites_RSMs(dataset, rsms, target_class_values, titles=None, 
    ...
    for subax, sub_targ_class_vals in zip(axes.flatten(), target_class_values):
        ...
        if sorting_latent in ["shape", "scale", "color"]:
