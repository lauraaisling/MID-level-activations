...
from . import data, plot_util, utils

# DEFAULT_LABELLED_FRACTIONS = [0.05, 0.1, 0.2, 0.4, 0.75, 1.0]
DEFAULT_LABELLED_FRACTIONS = [1.0]
...
class EncoderCore(nn.Module):
    def __init__(self, feat_size=84, input_dim=(1, 64, 64), vae=False, dropout=False, dropout_level=0.2):
        """
        ...
        if self.dropout: 
            self.feature_extractor = nn.Sequential(
                ...
                nn.Dropout(p=self.dropout_level), ###
...
def train_classifier(encoder, dataset, train_sampler, test_sampler, 
                     ...
                     test_color_bias_dataset=None,
                     dropout=False, dropout_level=0.2
                     ):
    """
    ...
    if dropout: 
        classifier = nn.Sequential(
            nn.Linear(linear_input, dataset.num_classes),
    ...
    if test_color_bias_dataset: 
        test_dataloader = torch.utils.data.DataLoader(
            test_color_bias_dataset, batch_size=batch_size, sampler=test_sampler
            )
    else: 
        test_dataloader = torch.utils.data.DataLoader(
    ...
    loss_arr = []
    for epoch in tqdm(range(num_epochs), disable=not(progress_bar)):
        total_loss = 0
        num_total = 0
        for iter_data in train_dataloader:
            ...
            # if num_total < 20:
            #     print(f"epoch {epoch}; y: {y}, ypred: {predicted_y_logits}")
...
class VAE_decoder(nn.Module):
    def __init__(self, feat_size=84, output_dim=(1, 64, 64), final_out_channels=1):
        """
        ...
        self.decoder_conv = nn.Sequential(
              ...
              nn.BatchNorm2d(6, affine=False),
              nn.ConvTranspose2d(
                  in_channels=6, out_channels=final_out_channels, kernel_size=5, stride=1
                  )
        )
...
def train_vae(encoder, dataset, train_sampler, num_epochs=100, batch_size=500, 
              beta=1.0, use_cuda=True, verbose=False, keep_colors=False):
    """
    ...
    reset_encoder_device = get_model_device(encoder) # for later
    encoder = encoder.to(device)
    decoder = VAE_decoder(encoder.feat_size, encoder.input_dim, final_out_channels=encoder.input_dim[0]).to(device)
        ...
        for batch_idx, (X, _, _) in enumerate(train_dataloader):
            ...
            if verbose and epoch % 10 == 9 and batch_idx == 0:
                ...
                plot_util.plot_dsprite_image_doubles(
                    ...
                    title=title, keep_colors=keep_colors)
...
def train_clfs_by_fraction_labelled(encoder, dataset, train_sampler, 
    ...
    for i in tqdm(range(n_fractions)):
        ...
        classifier,  _, train_acc[i], test_acc[i] = train_classifier(
            ...
            dropout=False, dropout_level=dropout_level
            )
        # classifier_li.append(classifier)
...
def train_encoder_clfs_by_fraction_labelled(
    ...
    for e, encoder in enumerate(encoders):
        classifier, train_accs[e], test_accs[e] = train_clfs_by_fraction_labelled(
            encoder, dataset, train_sampler, test_sampler, 
    ...
    if plot_accuracies:
        return classifier_li, train_accs, test_accs, ax
    else:
        return classifier_li, train_accs, test_accs
    
 
def train_clfs_by_prop_biases(encoders_all, dataset, train_samplers, 
    test_samplers, prop_biases=None, num_epochs=10, # freeze_features=True, 
    batch_size=1000, subset_seed=None, use_cuda=True, encoder_labels=None, 
    plot_accuracies=True, ax=None, title=None, plot_chance=True, colors=["blue"], 
    markers=".", verbose=False, 
    dropout=False, dropout_level=0.2):
    """
    train_clfs_by_prop_bias(encoder, dataset, train_sampler, 
        test_sampler)
    ...
    Required args:
     - encoders_all (nn.Module): List of list of encoder network instances for extracting features. 
        Should have method get_features().
    - dataset (dSpritesTorchDataset): dSprites torch dataset
    - train_sampler (SubsetRandomSampler): Training dataset sampler.
    - test_sampler (SubsetRandomSampler): Test dataset sampler.
    ...
    Optional args:
    - prop_biases (list): List of fractions of the total biased data 
        added to training data to use for training. If None, 
        0.0 is used. (default: None)
    - num_epochs (int):Number of epochs over which to train the 
        classifiers, if full dataset is used (the number used is scaled 
        for each fraction). 
        (default: 10)
    ...
    - subset_seed (int): seed for selecting data subset, if applicable 
        (default: None)
    - use_cuda (bool): If True, cuda is used, if available. (default: True)
    - encoder_labels (str): Label for the encoder. (default: None)
    - plot_accuracies (bool): If True, the accuracies are plotted. 
        (default: True)
    - ax (plt Axis): pyplot axis on which to plot accuracies. If None, a new 
        axis is initalized. (default: None)
    ...
    - plot_chance (bool): if True, chance level classifier accuracy is 
        plotted. (default: False)
    - colors (str): colors to use when plotting the accuracies. 
        (default: "blue")
    - markers (str): markers to use when plotting the accuracies. (default: ".")
    - verbose (bool): If True, classification accuracy is printed. 
        (default: False)
    ...
    - train_acc (1D np array): final training accuracy for each fraction 
        labelled
    - test_acc (1D np array): final test accuracy for each proportion of bias
    """

    device = "cuda" if use_cuda and torch.cuda.is_available() else "cpu"

    if isinstance(prop_biases, (int, float)):
        prop_biases = [prop_biases]

    if prop_biases is None:
        prop_biases = 0.0
        prop_biases_str = ", ".join(
            [str(val) for val in prop_biases]
            )
        if verbose:
            print("Using the following default proportion of bias values: "
                f"{prop_biases_str}\n")

    if len(prop_biases) == 0:
        raise ValueError("Include at least one value in prop_biases.")

    if np.min(prop_biases) < 0 or np.max(prop_biases) > 1: ######################
        raise ValueError(
            "all prop_biases must be between (0, 1) (excl, incl)"
            )
    classifier_li = []
    train_accs_li = []
    test_accs_li = []

    for idx_bias, prop_bias in enumerate(prop_biases): 
        print(f"{prop_bias}% purposely biased")
        encoders = encoders_all[idx_bias]
        train_sampler = train_samplers[idx_bias]
        test_sampler = test_samplers[idx_bias]
        classifiers = []

        train_accs = [] #np.full(len(encoders), np.nan)
        test_accs = [] #np.full(len(encoders), np.nan)
        ...
        # num_epochs_use_all = num_epochs # [
            # int(np.ceil(num_epochs / np.sqrt(prop)))
            # for prop in prop_biases
            # ]

        # n_mods = len(encoders)
        for idx_encoder, encoder in tqdm(enumerate(encoders)):

            if verbose and encoder_labels is not None:
                encoder_label = encoder_labels[idx_encoder]
                # add_str = "" if freeze_features else " and encoders"
                print(f"{encoder_label[0].capitalize()}{encoder_label[1:]} "
                    f"encoder: training classifiers...")
                
            # train_sampler = train_samplers[idx_encoder]
            # test_sampler = train_samplers[idx_encoder]

            reset_encoder_device = get_model_device(encoder)
            encoder.to(device)
            # if not freeze_features: # obtain new fresh version
            #     encoder = copy.deepcopy(orig_encoder)
            classifier,  _, train_acc, test_acc = train_classifier(
                encoder, dataset, train_sampler, test_sampler, 
                num_epochs=num_epochs[idx_encoder], 
                fraction_of_labels=1.0, 
                subset_seed=subset_seed, freeze_features=True, 
                batch_size=batch_size, progress_bar=False, verbose=False, 
            ...
            color = colors[idx_encoder]
            marker=markers[idx_encoder]

            if plot_accuracies:
                if ax is None:
                    _, ax = plt.subplots(1)

                prop_biases = np.asarray(prop_biases)
                sorter = np.argsort(prop_biases)
                sorted_prop_biases = prop_biases[sorter].tolist()
                ...
                if encoder_label is not None:
                    training_label = f"{encoder_label} (training)"
                    test_label = f"{encoder_label} (test)"
                else:
                    training_label = "training"
                    test_label = "test"
                if idx_bias !=0:
                ...
                ax.plot(
                    sorted_prop_biases[idx_bias], train_acc, ls="dashed", 
                    label=training_label, color=color, marker=marker, markersize=8, 
                    alpha=0.4
                    )
                ax.plot(
                    sorted_prop_biases[idx_bias], test_acc, lw=3, 
                    label=test_label, color=color, marker=marker, markersize=8, 
                    alpha=0.8
                    )

                ax.set_xlabel("Proportion of intentionally bias data used ") #(log scale)")
                ax.set_ylabel("Classification accuracy (%)")

                from matplotlib.ticker import ScalarFormatter
                # ax.set_xscale("log")
                ax.set_xticks(sorted_prop_biases)
                if len(sorted_prop_biases) < 8:
                    ax.set_xticklabels(sorted_prop_biases)
                ax.xaxis.set_major_formatter(ScalarFormatter())

                if title is not None:
                    ax.set_title(title)

        classifier_li.append(classifiers)
        train_accs_li.append(train_accs)
        test_accs_li.append(test_accs)

    ax.legend()


    return classifier_li, train_accs_li, test_accs_li


def train_encoder_clfs_by_prop_biases(
    encoders, dataset, train_samplers, test_samplers, prop_biases=None, 
    num_epochs=[10], batch_size=1000, subset_seed=None, # freeze_features=True, 
    use_cuda=True, encoder_labels=None, plot_accuracies=True, title=None, 
    verbose=False, 
    dropout=False, dropout_level=0.2):

    """
    train_encoder_clfs_by_prop_biases(encoder, train_sampler, 
        test_sampler)

    Trains classifiers on encoders, and returns training and test accuracy 
    with different proportion of intentionally biased data. Optionally plots the results.
    ...
    Optional args:
    - prop_biases (list): List of intentionally biased proportions of the 
        data to use for training. If None, 
        unbiased data is used. (default: None)
    - num_epochs (int or list): Number of epochs over which to train the 
        classifiers for each encoder, if full dataset is used (the number 
        used is scaled for each fraction). (default: 10)
    - freeze_features (bool or list): If True, the feature encoder is frozen 
    ...
    Returns: 
    - train_accs (2D np array): final training accuracies for each 
        encoder x fraction intentionally biased
    - test_accs (2D np array): final test accuracies for each 
        encoder x fraction intentionally biased
    if plot_accuracies:
    - ax (plt Axis): pyplot axis on which the accuracies are plotted
    ...
    if len(colors) < len(encoders[0]):
        raise NotImplementedError(
            f"Too may encoders ({len(encoders[0])}) for the number of "
            f"preset colors ({len(colors)})."
            )

    if isinstance(num_epochs, list):
        if len(num_epochs) != len(encoders[0]):
            raise ValueError("If providing num_epochs as a list, must "
                "provide as many as the number of encoders.")
    else:
        num_epochs = [num_epochs] * len(encoders[0])
    ...
    if prop_biases is None:
        ...
        if verbose:
            print("Using the following default proportion bias values: "
            f"{prop_biases_str}\n")
    ...
    if isinstance(encoder_labels, list):
        if len(encoder_labels) != len(encoders[0]):
            raise ValueError("If providing encoder_labels, must provide as "
                "many as the number of encoders.")
    else:
        encoder_labels = [None] * len(encoders[0])
    ...
    # classifier_li = []
    # train_accs = np.full((len(encoders[0]), len(prop_biases)), np.nan)
    # test_accs = np.full((len(encoders[0]), len(prop_biases)), np.nan)
    # for e, encoder in enumerate(encoders):
    classifier_li, train_accs_li, test_accs_li = train_clfs_by_prop_biases(
        encoders, dataset, train_samplers, test_samplers, 
        prop_biases=prop_biases, plot_accuracies=plot_accuracies, 
        batch_size=batch_size, verbose=verbose, # freeze_features=freeze_features[e], 
        subset_seed=subset_seed, use_cuda=use_cuda, 
        encoder_labels=encoder_labels, num_epochs=num_epochs, 
        ax=ax, plot_chance=False, colors=colors, markers=markers, 
        dropout=dropout, dropout_level=dropout_level
        )
    # classifier_li.append(classifiers)
    
    if plot_accuracies:
        return classifier_li, train_accs_li, test_accs_li, ax
    else:
        return classifier_li, train_accs_li, test_accs_li
    

def finetune_classifier(encoder, classifier, dataset, train_sampler, test_sampler, 
                     num_epochs=10, fraction_of_labels=1.0, batch_size=1000, 
                     freeze_features=True, subset_seed=None, use_cuda=True, 
                     progress_bar=True, verbose=False, 
                     test_color_bias_dataset=None,
    ...
    """
    finetune_classifier(encoder, classifier, dataset, train_sampler, test_sampler)
    ...
    - encoder (nn.Module): Encoder network instance for extracting features. 
        Should have method get_features(). If None, an Identity module is used.
    - classifier (nn.Linear): Classifier already trained to finetune. 
    - dataset (dSpritesTorchDataset): dSprites torch dataset
    - train_sampler (SubsetRandomSampler): Training dataset sampler.
    - test_sampler (SubsetRandomSampler): Test dataset sampler.
    ...
    for epoch in tqdm(range(num_epochs), disable=not(progress_bar)):
        ...
        # Reset to train since evaluating each epoch!!!
        classifier.train()
        reset_encoder_training = encoder.training
        if not freeze_features:
            encoder.train()
    ...
    train_acc, test_acc = accuracies#[-1]
    ...
    return encoder, classifier, loss_arr, train_acc, test_acc
